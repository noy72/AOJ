...
// 円と円の接線を求める
vector<Point> getTangentPointointCc(Circle a, Circle b, bool x = 0) {
	auto pow = [](double a) -> double {return a * a; };
		...
		rep(i,n){
			range(j,i + 1,n){
				for(auto k : getTangentPointointCc(c[i], c[j])){
					pair<Point,Point> p = getTangentPointointCp(c[j], k);
					if(k != p.first) maxi = max(maxi, getStones(c, Line(k, p.first)));
					if(k != p.second) maxi = max(maxi, getStones(c, Line(k, p.second)));
				}
			}
		}
		rep(i,n){
