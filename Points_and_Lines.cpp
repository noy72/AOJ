...
P factor(State &begin) {
	if (*begin == '(') {
		begin++; // '('を飛ばす。
		P ret = expression(begin);
		begin++; // ')'を飛ばす。
		return ret;
	} else {
		P res;
		res = make_pair(point(begin), Point(INF,INF));
		return res;
	}
}

// 四則演算の式をパースして、その評価結果を返す。
P expression(State &begin) {
	P ret = factor(begin);
	...
	for (;;) {
		if (*begin == '@') {
			...
			P p = factor(begin);
			if(isPoint(ret) and isPoint(p)){
				ret = P(ret.first, p.first);
			}else if(isPoint(ret) and not isPoint(p)){
				ret = P(reflect(Segment{p.first, p.second}, ret.first), Point(INF,INF));
			}else if(not isPoint(ret) and isPoint(p)){
				ret = P(reflect(Segment{ret.first, ret.second}, p.first), Point(INF,INF));
			}else{
				ret = P(intersectionL(Line{ret.first, ret.second}, Line{p.first, p.second}), Point(INF,INF));
			}
		} else {
			break;
		}
...
int main(){
	...
	while(cin >> s, s != "#"){
		State tmp = s.begin();
		P res = expression(tmp);
		cout << fixed << setprecision(10) << res.first.real() << ' ' << res.first.imag() << endl;
	}
}
