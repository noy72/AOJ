...
int main(){
	double w, h, v, t, x, y, p, q;
	cin >> w >> h >> v >> t >> x >> y >> p >> q;

	vector<Point> ps = {Point{p, q}};
	ps.emplace_back(reflect(Segment(Point(0, h), Point(2 * w, h)), ps[0]));
	rep(i,2){
		ps.emplace_back(reflect(Segment(Point(w, 0), Point(w, 2 * h)), ps[i]));
	}
