...
int main(){
	...
	while(cin >> n, n){
		...
		rep(i,n){
			Point pre = p[(i - 1 + n) % n], cur = p[i], nxt = p[(i + 1) % n];
			bisector.emplace_back(bisectorOfAngle(pre, cur, nxt));
		}

		vector<Point> ps;
		rep(i,bisector.size()){
			Line cur = bisector[i], nxt = bisector[(i + 1) % bisector.size()];
			ps.emplace_back(intersectionL(cur, nxt));
		}
		...
		rep(i,ps.size()){
			double mini = 1e9;
			rep(j,n){
				Point cur = p[j], nxt = p[(j + 1) % n];
				mini = min(mini, distanceLPoint(cur, nxt, ps[i]));
			}
			ans = max(ans, mini);
		}
