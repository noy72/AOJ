...
int main(){
    ...
    rep(i,n){
        events.push_back(make_pair(-c[i].r + c[i].c.real(), i));
        events.push_back(make_pair(c[i].r + c[i].c.real(), i + n));
    }
    sort(all(events));
    ...
    rep(i,events.size()){
        ...
        if(events[i].second < n){
            set<pair<double,int> >::iterator it = outers.lower_bound(make_pair(c[id].c.imag(), id));
            if(it != outers.end() && isInside(c[id], c[it->second])) continue;
            if(it != outers.begin() && isInside(c[id], c[(--it)->second])) continue;
            res.push_back(id);
            outers.insert(make_pair(c[id].c.imag(), id));
        }else{
            outers.erase(make_pair(c[id].c.imag(), id));
        }
    }
