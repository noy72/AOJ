...
//凸多角形と直線の交差判定
bool isIntersectedPl(Polygon p, Line l){
	rep(i,p.size()){
		if(isIntersectedL(Line{p[i], p[(i + 1) % p.size()]}, l)) return true;
	}
	return false;
}
...
bool containSamePoints(vector<Point>& crs, Point p){
	for(auto i : crs){
		if(abs(i - p) < EPS) return true;
	}
	return false;
}
...
int main(){
	...
	while(cin >> n, n){
		...
		while(n--){
			double lx, ly, rx, ry;
			cin >> lx >> ly >> rx >> ry;
			Line l = Line{Point{lx, ly}, Point{rx, ry}};

			ans++;
			vector<Point> crs;
			rep(i,ls.size()){
				if(isIntersectedLs(ls[i], l) and not isContainSamePoints(ls[i], l)){
					Point p = intersectionLs(ls[i], l);
					if(not containSamePoints(crs, p)){
						crs.emplace_back(p);
					}
				}
			}
