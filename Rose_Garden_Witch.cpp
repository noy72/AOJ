...
int main(){
	...
	rep(i,h){
		rep(j,w){
			...
			int lx = j, ly = i, rx = j + 1, ry = i + 1;
			ma[Line(Point(lx, ly), Point(lx, ry))]++;
			ma[Line(Point(lx, ry), Point(rx, ry))]++;
			ma[Line(Point(rx, ry), Point(rx, ly))]++;
			ma[Line(Point(rx, ly), Point(lx, ly))]++;
		}
	}

	vector<Line> ls;
	for(auto p : ma){
		if(p.second == 1) ls.emplace_back(p.first);
	}
